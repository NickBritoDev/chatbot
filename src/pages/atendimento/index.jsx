  ...
  React.useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        socket.on(`notificacao-painel-atendimento-cliente-${roomKey}`, (notification, undefined) => {
          checkPageStatus(notification.message, undefined)
        })
      } else {
          ...
          <Card overflowX={'hidden'} boxShadow={'none'} pos={{ base: 'absolute', md: 'initial' }} left={{ base: 0 }} top={{ base: 0 }} h={{ base: '100vh', md: '90vh' }} w={'100%'} mt={{ base: 0, md: 10 }} >
            ...
            <CardBody mt={6} borderTop={'1px solid #ccc'} >
              {informacao.status === 'Em Trilha'
                ? (
                  <>
                    <TrilhasAtendimento codePage={codePage} />
                  </>
                  )
                : (
                  <>
